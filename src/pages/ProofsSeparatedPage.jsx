...
export default function ProofsSeparatedPage() {
  ...
  async function review(action) {
    ...
    if (!REVIEWABLE.includes(String(selected.status).toUpperCase())) { setMessage('Este comprovativo já foi processado.'); return; }
  ...
  return <div className="space-y-5">
    ...
    <section className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]">
      ...
      <aside className="card-premium p-5 xl:sticky xl:top-24 xl:self-start">{selected ? <div className="space-y-4"><div><p className="text-xs font-black uppercase tracking-wide text-blue-700">{chargeCategoryLabel(selected.chargeCategory)}</p><h2 className="mt-2 text-lg font-black text-slate-950 dark:text-white">{selected.proofCode}</h2></div><Details item={selected} /><button onClick={() => openPaymentProofAttachment(selected.id)} className="btn-secondary w-full justify-center" disabled={working}><Eye size={16} /> Abrir anexo</button><textarea className="input-premium min-h-28 resize-y" value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Observação da DCR ou motivo da rejeição..." />{canValidate ? <div className="grid grid-cols-2 gap-3"><button onClick={() => review('approve')} disabled={working} className="btn-primary justify-center"><CheckCircle2 size={16} /> Aprovar</button><button onClick={() => review('reject')} disabled={working} className="btn-secondary justify-center text-red-700"><XCircle size={16} /> Rejeitar</button></div> : <p className="text-xs font-semibold text-slate-500">Perfil em modo de consulta.</p>}</div> : <EmptyState title="Selecione um comprovativo" description="Consulte a categoria e os dados da cobrança." />}</aside>
    </section>
  </div>;
}
