...
export default function AcademicServiceOrdersPage() {
  ...
  async function confirmOrderPayment() {
    ...
    try {
      ...
      setSuccess('Pagamento confirmado. O recibo digital foi emitido e o envio ao estudante foi processado; o pedido foi liberado para a Secretaria.');
            ...
            <div className="space-y-5 p-5">
              ...
              <div className="premium-card p-4">
                ...
                <div className="mt-4 space-y-3">
                  ...
                  {selected.status === 'ASSINADO' && canProcess ? <><Field label="Local de levantamento"><input value={physicalLocation} onChange={(event) => setPhysicalLocation(event.target.value)} className="input-premium" /></Field><ActionButton icon={MapPin} label="Confirmar disponibilidade e liberar notificações" onClick={() => runAction('pickup', (id) => markAcademicServiceOrderReadyForPickup(id, { physicalLocation }), 'Documento disponível fisicamente. As notificações por WhatsApp e e-mail foram liberadas.')} loading={busy === 'pickup'} success /></> : null}
                  {selected.status === 'ASSINADO' && !canProcess ? <ReadOnlyHint text="Documento assinado. A próxima ação pertence à Secretaria: confirmar a disponibilidade física e, em seguida, notificar o estudante por WhatsApp e e-mail." /> : null}
                  {selected.status === 'PRONTO_PARA_LEVANTAMENTO' && canProcess ? <ActionButton icon={Send} label="Enviar aviso por WhatsApp + e-mail" onClick={() => runAction('whatsapp', sendAcademicServiceOrderPickupWhatsapp, 'WhatsApp confirmado e envio complementar por e-mail processado.')} loading={busy === 'whatsapp'} success /> : null}
                  {selected.status === 'PRONTO_PARA_LEVANTAMENTO' && !canProcess ? <ReadOnlyHint text="O documento está disponível. A Secretaria deve enviar agora o aviso de levantamento por WhatsApp e e-mail." /> : null}
                  {selected.status === 'WHATSAPP_ENVIADO' && !canProcess ? <ReadOnlyHint text="A notificação de levantamento já foi processada pela Secretaria. O pedido aguarda o registo da entrega física." /> : null}
