...
export default function AcademicDocumentsPage() {
  ...
  async function createDocument(event) {
    ...
    try {
      ...
      setSuccess('Declaração de demonstração criada. Revise o texto antes de encaminhar para assinatura.');
      setSelected(created);
  ...
  async function runAction(action, successMessage) {
    ...
    setSuccess('');
    try {
      const updated = await action.fn(selected.id);
      setSelected(updated);
  ...
  function preview(item = selected) {
    if (!item?.pdfUrl) return;
    window.open(`${item.pdfUrl}?v=${Date.now()}`, '_blank', 'noopener,noreferrer');
  }
    ...
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-amber-300/70 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-900 shadow-sm dark:border-amber-400/20 dark:bg-amber-400/10 dark:text-amber-100">
        <div className="flex items-start gap-3">
          ...
          <div><p className="font-black">Demonstração — sem validade jurídica.</p><p className="mt-1 text-xs leading-5">Nesta fase, todas as declarações usam a assinatura eletrónica demonstrativa de Zakeu António Zengo, Presidente da Instituição. Os responsáveis reais serão configurados posteriormente.</p></div>
        </div>
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[.82fr_1.18fr]">
        <div className="premium-card p-5">
          <div className="flex items-start justify-between gap-4">
            <div><p className="text-xs font-extrabold uppercase tracking-[.14em] text-[#3157D5] dark:text-[#8EA9FF]">Nova demonstração</p><h2 className="mt-1 text-lg font-extrabold text-imetro-navy dark:text-white">Criar declaração simples</h2><p className="mt-1 text-xs font-medium leading-5 text-slate-500 dark:text-slate-300">Use a matrícula para preencher automaticamente os dados académicos.</p></div>
          ...
          <form onSubmit={createDocument} className="mt-5 space-y-4">
            <Field label="Matrícula do estudante">
              <input required value={createForm.studentNumber} onChange={(event) => setCreateForm((current) => ({ ...current, studentNumber: event.target.value }))} className="input-premium" placeholder="Ex.: 202301404" disabled={!canPrepare || Boolean(busy)} />
            </Field>
            <Field label="Finalidade">
              <input required value={createForm.purpose} onChange={(event) => setCreateForm((current) => ({ ...current, purpose: event.target.value }))} className="input-premium" placeholder="Ex.: Comprovação académica" disabled={!canPrepare || Boolean(busy)} />
            </Field>
            <Field label="Texto personalizado (opcional)">
              <textarea value={createForm.declarationText} onChange={(event) => setCreateForm((current) => ({ ...current, declarationText: event.target.value }))} rows={5} className="input-premium min-h-[126px] resize-y leading-6" placeholder="Deixe em branco para usar o texto institucional automático." disabled={!canPrepare || Boolean(busy)} />
            </Field>
            {canPrepare ? (
              <button disabled={Boolean(busy)} className="btn-primary w-full justify-center">
                {busy === 'create' ? <Loader2 className="animate-spin" size={17} /> : <FilePenLine size={17} />} Criar declaração
              </button>
            ) : <ReadOnlyHint text="Seu perfil possui acesso de consulta, mas não pode preparar documentos." />}
          </form>
        </div>
        ...
        <div className="premium-card overflow-hidden">
          <div className="border-b border-slate-200 p-4 dark:border-white/10">
            <div className="grid gap-3 md:grid-cols-[1fr_220px]">
              ...
              <select value={status} onChange={(event) => setStatus(event.target.value)} className="input-premium">
          ...
          <div className="hidden overflow-x-auto lg:block">
            <table className="w-full table-fixed text-left">
              <thead className="bg-slate-50 text-[10px] font-extrabold uppercase tracking-[.12em] text-slate-500 dark:bg-white/[.03] dark:text-slate-400">
                <tr><th className="w-[30%] px-4 py-3">Estudante</th><th className="w-[25%] px-4 py-3">Documento</th><th className="w-[21%] px-4 py-3">Estado</th><th className="w-[24%] px-4 py-3">Atualização</th></tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-white/[.06]">
                {filtered.map((item) => <DocumentRow key={item.id} item={item} onClick={() => openDocument(item)} />)}
              </tbody>
            </table>
          </div>

          <div className="space-y-3 p-4 lg:hidden">
            {filtered.map((item) => <DocumentCard key={item.id} item={item} onClick={() => openDocument(item)} />)}
          </div>
        ...
        <div className="fixed inset-0 z-[80] flex justify-end">
          ...
          <aside className="relative h-full w-full max-w-[620px] overflow-y-auto border-l border-slate-200 bg-white shadow-2xl dark:border-white/10 dark:bg-[#081321]">
            <div className="sticky top-0 z-10 border-b border-slate-200 bg-white/95 px-5 py-4 backdrop-blur dark:border-white/10 dark:bg-[#081321]/95">
              <div className="flex items-start justify-between gap-4">
                <div><p className="text-xs font-extrabold uppercase tracking-[.14em] text-[#3157D5] dark:text-[#8EA9FF]">Declaração simples</p><h2 className="mt-1 text-xl font-extrabold text-imetro-navy dark:text-white">{selected.studentName}</h2><p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-300">{selected.studentNumber} · {selected.documentCode}</p></div>
                <button onClick={() => setSelected(null)} className="rounded-xl p-2 text-slate-500 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-white/10"><X size={20} /></button>
              </div>
            </div>
            ...
            <div className="space-y-5 p-5">
              ...
              <div className="premium-card p-4">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <ActionButton icon={FileText} label="Pré-visualizar PDF" onClick={() => preview()} disabled={Boolean(busy)} />
                  {selected.validationUrl ? <ActionButton icon={ShieldCheck} label="Abrir validação" onClick={() => window.open(selected.validationUrl, '_blank', 'noopener,noreferrer')} disabled={Boolean(busy)} /> : null}

                  {editable && canPrepare ? <ActionButton icon={UserRoundCheck} label="Pronto para assinatura" onClick={() => runAction({ name: 'ready', fn: markAcademicDocumentReady }, 'Documento encaminhado para assinatura.') } loading={busy === 'ready'} disabled={Boolean(busy)} primary /> : null}
                  {selected.status === 'READY_FOR_SIGNATURE' && canSign ? <ActionButton icon={FileSignature} label="Assinar como Zakeu" onClick={() => runAction({ name: 'sign', fn: signAcademicDocumentDemo }, 'Declaração assinada eletronicamente em modo demonstração.') } loading={busy === 'sign'} disabled={Boolean(busy)} warning /> : null}
                  {['SIGNED', 'SENT'].includes(selected.status) && canSend ? <ActionButton icon={Send} label={selected.status === 'SENT' ? 'Reenviar no WhatsApp' : 'Enviar no WhatsApp'} onClick={() => runAction({ name: 'send', fn: sendAcademicDocumentWhatsapp }, 'Documento disponibilizado ao estudante pelo WhatsApp.') } loading={busy === 'send'} disabled={Boolean(busy)} success /> : null}
                </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs font-semibold leading-5 text-slate-600 dark:border-white/10 dark:bg-white/[.04] dark:text-slate-300">
                <p><b>Criado:</b> {formatDateTime(selected.createdAt)}</p>
                <p><b>Assinado:</b> {formatDateTime(selected.signedAt)}</p>
                <p><b>Enviado:</b> {formatDateTime(selected.sentAt)}</p>
              </div>
            </div>
          </aside>
        </div>
...
function Notice({ tone, icon: Icon, text }) {
  ...
  return <div className={`flex items-start gap-3 rounded-2xl border px-4 py-3 text-sm font-semibold ${tones}`}><Icon size={18} className="mt-0.5 shrink-0" /><span>{text}</span></div>;
}
...
function ActionButton({ icon: Icon, label, onClick, loading, disabled, primary, warning, success }) {
  const tone = primary
    ? 'bg-[#3157D5] text-white hover:bg-[#2449C6]'
    : warning
      ? 'bg-amber-500 text-slate-950 hover:bg-amber-400'
      : success
        ? 'bg-emerald-600 text-white hover:bg-emerald-500'
        : 'border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 dark:border-white/10 dark:bg-white/[.04] dark:text-white dark:hover:bg-white/[.08]';
