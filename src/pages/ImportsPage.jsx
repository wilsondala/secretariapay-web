...
function BatchCard({ batch, selected, onSelect }) {
  return (
    <button
      ...
      className={`w-full rounded-3xl border p-5 text-left transition hover:-translate-y-0.5 hover:shadow-soft ${
    ...
    >
      ...
      <div className="mt-4 grid grid-cols-4 gap-2 text-center">
        <div className="rounded-2xl bg-slate-50 p-2">
          <p className="text-lg font-black text-slate-950">{batch.totalRows}</p>
          <p className="text-[11px] text-slate-500">Total</p>
        </div>
        <div className="rounded-2xl bg-emerald-50 p-2">
          <p className="text-lg font-black text-emerald-700">{batch.validRows}</p>
          <p className="text-[11px] text-emerald-700">Válidas</p>
        </div>
        <div className="rounded-2xl bg-red-50 p-2">
          <p className="text-lg font-black text-red-700">{batch.invalidRows}</p>
          <p className="text-[11px] text-red-700">Inválidas</p>
        </div>
        <div className="rounded-2xl bg-blue-50 p-2">
          <p className="text-lg font-black text-blue-700">{batch.syncedRows}</p>
    ...
    <div className="space-y-7">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black text-slate-950">Importações académicas</h1>
        ...
        <div className="flex flex-wrap gap-3">
          <button
            onClick={loadBatches}
            className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-700 shadow-soft transition hover:-translate-y-0.5"
          >
            <RefreshCw size={18} /> Atualizar
          </button>
          <button className="inline-flex items-center gap-2 rounded-2xl bg-imetro-navy px-4 py-3 text-sm font-bold text-white shadow-soft transition hover:-translate-y-0.5">
          ...
          <div className="flex flex-col gap-3 md:flex-row">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-slate-200 py-3 pl-11 pr-4 text-sm outline-none transition focus:border-imetro-gold"
              />
            </div>
            <select
              value={statusFilter}
              onChange={(event) => setStatusFilter(event.target.value)}
              className="rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold outline-none focus:border-imetro-gold"
            >
            ...
            <>
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <h2 className="mt-1 text-xl font-black text-slate-950">{selectedBatch.importCode}</h2>
                  <p className="mt-1 text-sm text-slate-500">{selectedBatch.notes || 'Sem observações.'}</p>
                </div>
                <button
                  onClick={handleSync}
                  disabled={syncing}
                  className="inline-flex items-center justify-center gap-2 rounded-2xl bg-imetro-gold px-4 py-3 text-sm font-black text-imetro-navy shadow-soft transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60"
                >
              ...
              <div className="mt-5 overflow-hidden rounded-3xl border border-slate-200">
                ...
                {rowsLoading ? (
                  <div className="p-6"><LoadingState title="Carregando linhas" /></div>
                ) : filteredRows.length ? (
                  ...
                  <div className="p-6"><EmptyState title="Nenhuma linha encontrada" message="Ajuste a busca ou o filtro de estado." /></div>
                )}
              </div>
            </>
          ) : (
