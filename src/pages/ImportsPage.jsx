...
export default function ImportsPage() {
  ...
  async function loadBatchDetails(batchId) {
    ...
    setRowsLoading(true);
    try {
      const [rowsResult, historyResult] = await Promise.all([
        fetchImportRows(batchId),
        fetchImportHistory(batchId),
      ]);
      ...
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-[#061936] via-[#08285A] to-[#061936] p-5 text-white sm:p-7">
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-4 text-2xl font-black sm:text-4xl">Importações académicas</h1>
            <p className="mt-3 max-w-3xl text-sm font-medium leading-6 text-white/80 sm:text-base">
              Importe CSV ou Excel, corrija linhas inválidas, acompanhe o histórico e sincronize sem duplicidade.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button onClick={() => loadBatches(selectedBatch?.id)} className="inline-flex items-center gap-2 rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-sm font-black">
              <RefreshCw size={18} /> Atualizar
            </button>
            <button onClick={() => setUploadOpen(true)} className="inline-flex items-center gap-2 rounded-2xl bg-imetro-gold px-4 py-3 text-sm font-black text-imetro-navy">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,.95fr)_minmax(0,1.35fr)]">
        <section className="card-premium min-w-0 p-4">
          <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_190px]">
            <div className="relative"><Search className="absolute left-4 top-3.5 text-slate-400" size={18} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar lote, código, ficheiro..." className="input-premium pl-11" /></div>
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="input-premium">
              <option value="ALL">Todos</option><option value="DRAFT">Rascunho</option><option value="VALIDATED">Validado</option><option value="COMPLETED">Concluído</option><option value="CANCELLED">Cancelado</option>
            </select>
          </div>
          <div className="mt-5 space-y-4">
            {filteredBatches.length
              ? filteredBatches.map((batch) => <BatchCard key={batch.id} batch={batch} selected={selectedBatch?.id === batch.id} onSelect={handleSelect} />)
          ...
          {selectedBatch ? <>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <h2 className="mt-2 text-base font-black text-slate-950 dark:text-white">{selectedBatch.importCode}</h2>
                <p className="mt-1 text-sm font-semibold text-slate-500 dark:text-slate-300">{selectedBatch.notes || 'Sem observações.'}</p>
              </div>
              <div className="flex flex-wrap gap-2">
                <ActionButton onClick={() => setHistoryOpen(true)}><History size={17} /> Histórico</ActionButton>
                {history?.canReprocess && <ActionButton onClick={handleReprocess} disabled={processing}><RotateCcw size={17} /> Reprocessar</ActionButton>}
                {history?.canCancel && <ActionButton danger onClick={handleCancel} disabled={processing}><Ban size={17} /> Cancelar lote</ActionButton>}
                {Number(selectedBatch.invalidRows || 0) > 0 && <ActionButton onClick={() => downloadImportErrors(selectedBatch.id, selectedBatch.importCode)}><Download size={17} /> Relatório de erros</ActionButton>}
                {['DRAFT', 'PENDING_VALIDATION', 'VALIDATED'].includes(selectedStatus) && <ActionButton onClick={() => runBatchAction(validateImportBatch, 'Validação concluída.')} disabled={processing}>Revalidar</ActionButton>}
                {selectedStatus === 'VALIDATED' && Number(selectedBatch.invalidRows || 0) === 0 && <ActionButton onClick={() => runBatchAction(completeImportBatch, 'Lote concluído.')} disabled={processing}>Concluir staging</ActionButton>}
                {(selectedStatus === 'COMPLETED' || (selectedStatus === 'VALIDATED' && Number(selectedBatch.invalidRows || 0) === 0)) && <ActionButton primary onClick={() => runBatchAction(syncImportBatch, 'Sincronização concluída.')} disabled={processing}><DatabaseZap size={18} /> Sincronizar</ActionButton>}
              </div>
            </div>
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-[minmax(0,1fr)_190px]">
              <div className="relative"><Search className="absolute left-4 top-3.5 text-slate-400" size={18} /><input value={rowSearch} onChange={(e) => setRowSearch(e.target.value)} placeholder="Buscar aluno, matrícula, curso, turma..." className="input-premium pl-11" /></div>
            ...
            <div className="mt-5">
              {rowsLoading ? <LoadingState title="Carregando linhas" /> : filteredRows.length
                ? <div className="space-y-3">{filteredRows.map((row) => <RowCard key={row.id} row={row} onEdit={() => setEditingRow(row)} />)}</div>
                : <EmptyState title="Nenhuma linha encontrada" message="Ajuste a busca ou o filtro de estado." />}
            </div>
          </> : <EmptyState title="Nenhum lote selecionado" message="Selecione uma importação WebSchool para ver as linhas." />}
        </section>
      </div>

      {uploadOpen && <UploadModal onClose={() => setUploadOpen(false)} suggestedInstitutionId={selectedBatch?.institutionId || batches.find((batch) => batch.institutionId)?.institutionId || ''} onUploaded={async (batch) => { setUploadOpen(false); await loadBatches(batch.id); }} />}
      {editingRow && <EditRowModal row={editingRow} batchId={selectedBatch.id} onClose={() => setEditingRow(null)} onSaved={async () => { setEditingRow(null); await loadBatches(selectedBatch.id); }} />}
...
function EditRowModal({ row, batchId, onClose, onSaved }) {
  ...
  async function submit(event) {
    ...
    try { await updateImportRow(batchId, row.id, form); await onSaved(); }
    catch (err) { setError(readError(err, 'Não foi possível corrigir a linha.')); }
