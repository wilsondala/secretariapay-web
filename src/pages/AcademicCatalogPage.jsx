  ...
  return <div className="space-y-5">
    <section className="rounded-3xl bg-gradient-to-r from-[#061936] via-[#08285A] to-[#061936] p-6 text-white">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex gap-2"><button onClick={load} className="inline-flex items-center gap-2 rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-sm font-black"><RefreshCw size={17}/> Atualizar</button><button onClick={() => setEditing({ type: tab === 'courses' ? 'course' : 'class' })} className="inline-flex items-center gap-2 rounded-2xl bg-imetro-gold px-4 py-3 text-sm font-black text-imetro-navy"><Plus size={17}/> Novo {tab === 'courses' ? 'curso' : 'turma'}</button></div>
      </div>
    </section>
    ...
    <section className="card-premium p-4">
      ...
      {loading ? <div className="flex items-center justify-center py-16"><Loader2 className="animate-spin"/></div> : tab === 'courses' ? <div className="mt-5 grid gap-3 xl:grid-cols-2">{visibleCourses.map(item=><CourseCard key={item.id} item={item} onEdit={()=>setEditing({type:'course', item})}/>)}</div> : <div className="mt-5 grid gap-3 xl:grid-cols-2">{visibleClasses.map(item=><ClassCard key={item.id} item={item} onEdit={()=>setEditing({type:'class', item})}/>)}</div>}
    </section>
    {editing && <EditModal mode={editing.type} item={editing.item} courses={courses} onClose={()=>setEditing(null)} onSaved={async()=>{setEditing(null);await load();}}/>}
  </div>;
}
...
function CourseCard({item,onEdit}){return <div className="rounded-3xl border border-slate-100 bg-white p-5 dark:border-white/10 dark:bg-[#0D1B2E]"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wide text-slate-400">{item.code||'Sem código'}</p><h3 className="mt-2 text-lg font-black text-slate-950 dark:text-white">{item.name}</h3><p className="mt-1 text-sm font-semibold text-slate-500">{item.faculty||'Sem faculdade'} · {item.durationYears||'-'} anos</p></div><button onClick={onEdit} className="rounded-xl border border-slate-200 p-2 dark:border-white/10"><Pencil size={16}/></button></div><span className={`mt-4 inline-flex rounded-full px-3 py-1 text-xs font-black ${item.active?'bg-emerald-500/10 text-emerald-600':'bg-slate-500/10 text-slate-500'}`}>{item.active?'Ativo':'Inativo'}</span></div>}
function ClassCard({item,onEdit}){return <div className="rounded-3xl border border-slate-100 bg-white p-5 dark:border-white/10 dark:bg-[#0D1B2E]"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wide text-slate-400">{item.courseName}</p><h3 className="mt-2 text-lg font-black text-slate-950 dark:text-white">{item.name}</h3><p className="mt-1 text-sm font-semibold text-slate-500">{item.academicYear} · {shiftLabel(item.shift)} · {item.yearLevel||'-'}º ano</p></div><button onClick={onEdit} className="rounded-xl border border-slate-200 p-2 dark:border-white/10"><Pencil size={16}/></button></div><span className={`mt-4 inline-flex rounded-full px-3 py-1 text-xs font-black ${item.active?'bg-emerald-500/10 text-emerald-600':'bg-slate-500/10 text-slate-500'}`}>{item.active?'Ativa':'Inativa'}</span></div>}
