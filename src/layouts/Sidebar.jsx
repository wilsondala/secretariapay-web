...
const groups = [
  {
    label: 'Visão geral',
    items: [['Painel', '/dashboard', Home, 'Resumo executivo']],
  },
  {
...
function NavItem({ label, path, Icon, helper, onClose, collapsed }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        [
          'group flex items-center rounded-2xl text-[14px] font-black transition duration-200',
    ...
    <div className="flex h-full flex-col overflow-hidden bg-gradient-to-b from-[#061936] via-[#071f42] to-[#030b18] text-white shadow-[28px_0_70px_rgba(7,20,45,.24)]">
      <div className={collapsed ? 'relative px-3 pb-4 pt-5' : 'relative px-5 pb-5 pt-7'}>
        ...
        <button
          ...
          className={[
            'relative mt-4 hidden w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[.07] px-3 py-2.5 text-xs font-black text-white/75 transition hover:bg-white/12 hover:text-white lg:inline-flex',
            collapsed ? 'px-2' : '',
          ].join(' ')}
      ...
      <nav className={collapsed ? 'flex-1 space-y-4 overflow-y-auto px-3 pb-5' : 'flex-1 space-y-6 overflow-y-auto px-4 pb-5'}>
        {groups.map((group) => (
          <div key={group.label}>
            {!collapsed ? <p className="mb-2 px-3 text-[11px] font-black uppercase tracking-[.18em] text-white/36">{group.label}</p> : null}
      ...
      <div className={collapsed ? 'px-3 pb-5' : 'px-4 pb-5'}>
        <div className="rounded-3xl border border-white/10 bg-white/[.07] p-4 shadow-[0_18px_42px_rgba(0,0,0,.18)]">
          <div className="flex items-center justify-between gap-3">
            ...
            {!collapsed ? <span className="rounded-full bg-white/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-wide text-white/70">Demo ativa</span> : null}
          </div>
          {!collapsed ? (
            <>
